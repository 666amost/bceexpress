...
export function CourierShipmentList({ courierId }: CourierShipmentListProps) {
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-4 md:w-3/4 border-t md:border-t-0 md:border-l">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    {/* === Bagian Tampilan Lokasi yang Diperbaiki === */}
                    <p className="text-sm text-muted-foreground">Latest Update</p>
                    {/* Tampilkan location jika ada, kalau tidak, cek latitude/longitude */}
                    <p className="font-medium">
                        {shipment.latest_update?.location ||
                         (shipment.latest_update?.latitude != null && shipment.latest_update?.longitude != null // Periksa null atau undefined
                           ? `${shipment.latest_update.latitude?.toFixed(6)}, ${shipment.latest_update.longitude?.toFixed(6)}` // Tampilkan koordinat jika lokasi kosong tapi koordinat ada
                           : "No location" // Jika location kosong dan koordinat kosong
                         )
                        }
                    </p>
                     {/* Jika perlu menampilkan Altitude (jika ada di database dan Anda tambahkan di select), tambahkan di sini */}
                     {/* Contoh: */}
                     {/* {shipment.latest_update?.altitude != null && (
                        <p className="text-sm text-muted-foreground">Altitude: {shipment.latest_update.altitude} m</p>
                     )} */}
                    {/* === Akhir Bagian Tampilan Lokasi === */}
